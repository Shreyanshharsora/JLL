...
Route::get('/custom-skyline/skyline-list',array('uses' => 'CustomSkylineController@getSkylineList'));
...
Route::get('/local-market-pdf/{file}',array('uses' => 'CitiesController@downloadPdf'));
...
$pageData = $pageData->groupBy('e_page_type');
    ...
    return View::make('index')->with(array('footer_link'=>$pageData, 'cms'=>'false','headerCountryData' => $countriesData,'userData'=>$userData, 'userCustomSkylines' => $userCustomSkylines));
});
...
Route::get('/{slug}', function($slug) {
    ...
    $page = DB::table('pages')->where('v_slug', $slug)->whereNull('deleted_at')->first();
    ...
    $pageData = $pageData->groupBy('e_page_type');
    		...
    		return View::make('page')->with(array('content'=>$page,'footer_link'=>$pageData, 'headerCountryData'=>$countriesData, 'cities'=>$citiesData, 'industryDataArray'=>$industryDataArray, 'cms'=>'true','userData'=>$userData, 'userCustomSkylines' => $userCustomSkylines));
...
Route::get('/city/{code}', function($code) {
    ...
    $userCustomSkylines = [];
    if(isset($userData->id) && count($userData) > 0){
        $userShareCustomSkylines = SkylineShareTo::where('v_email_id',$userData->email)->lists('i_skyline_id');
        if($userShareCustomSkylines){
            $userCustomSkylines=Skyline::where(function($query) use ($userShareCustomSkylines,$userData){
                    $query = $query->whereIn('id', $userShareCustomSkylines)
    ...
    if((count($city)) > 0) {
        ...
        if(!empty($countriesData)) {
            ...
            $pageData = Cms::where('status','=','1')->select('v_title','v_external_link','v_slug','e_show_in_menu','e_show_in_footer','i_order','id', 'e_page_type')->orderBy('i_order','ASC')->get();
            $pageData = $pageData->groupBy('e_page_type');
