...
app.service('PaginationService', function ($http, $rootScope, $location, $timeout, GLOBAL, mySharedService,$state, $cookieStore,$filter) {     
    ...
    this.change_active_inactive = function(url, id, status,$scope) {
        clearTimeout(timeoutID);
        //timeoutID = setTimeout(function() {
            if(status == '0'){
            ...
            if($state.current.data.currTab == 'home_page_content')
            {
               if(status == '0'){
                    $rootScope.updated_status = '0';
                    $rootScope.success_msg = 'Block is show only logged out user.';
                } else {
                    $rootScope.updated_status = '1';
                    $rootScope.success_msg = 'Block is show only logged in user.'; 
                }
            }
            $rootScope.$apply();
