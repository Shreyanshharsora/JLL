'use strict';
app.controller('DomainsController', function($rootScope, $scope, $http, $timeout, PaginationService, GLOBAL, mySharedService, $filter, $state, $stateParams) {
    ...
    $scope.$on('$viewContentLoaded', function() {
        ...
        PaginationService.loading(GLOBAL.API_URL+"domains","updated_at");
    ...
    /*if($stateParams.id != "" && $stateParams.id != undefined)
    {
        if(mySharedService.edit_fields != undefined) {
            ...
            var responsePromise = $http.post(GLOBAL.API_URL+"domains/data/"+$stateParams.id);
    ...
    $rootScope.export_to_excel = function(fields){
        ...
        window.location.href = GLOBAL.API_URL+'domains/export/'+$scope.someModelSerialized;
    }
    
    $rootScope.frm_add_open = function(parent_items){
        angular.element('.alert').hide();
        $("#domainAdd .form-group").removeClass('has-error');
        $("#domainAdd .required").removeAttr('style');
        $("#domainAdd .help-block-error").remove();
    ...
    $rootScope.submit_frm_add = function(add_fields, button_pressed){
        ...
        $('#domainAdd').find('#v_domain_name_error').hide();
        if(form_valid('#domainAdd',"help-block")){
            angular.element('div.loading').css('background','rgba(0,0,0,0.6)').removeClass('hide');
            //Call submit data services
            PaginationService.submit_add_form_data(GLOBAL.API_URL+"domains/add", add_fields,'','domain_page');
            $('#domainAdd').find('#v_domain_name_error').hide();
           /* if(button_pressed == 'New'){
                PaginationService.submit_add_form_data(GLOBAL.API_URL+"domains/add", add_fields);
            } else {
                PaginationService.submit_add_form_data(GLOBAL.API_URL+"domains/add", add_fields, '/domains');
            }*/
    ...
    /* Begin edit form */
    $rootScope.loadEditData = function(data, data_id){
        //$scope.domain_name = data;
        $scope.edit_data = {};
        $scope.edit_data.v_domain_name = data;
        $scope.edit_data.id = data_id;
        //mySharedService.edit_fields = $rootScope.edit_fields; 
        //$rootScope.$broadcast($rootScope.edit_fields1);
        /*$state.go('/domains-edit',{id: item.id});*/
        $rootScope.recordEditForm = {};
        $rootScope.recordEditForm['record_'+data_id] = data_id;
    }
    ...
    $scope.clearFormData = function($page){
        if($page == 'edit'){        
            ...
            setTimeout(function(){
                ...
                $('.domain_add').find('#v_domain_name_error').hide();
                $('.domain_add').find('#add_domain_name').css('border','1px solid rgb(229, 229, 229)');
    ...
    $rootScope.submit_frm_edit = function(edit_data){
        $('#frmEdit').find('#v_domain_name_error_edit').hide();
        if(form_valid('#frmEdit',"help-block")){
            
            angular.element('div.loading').css('background','rgba(0,0,0,0.6)').removeClass('hide');
            //Call submit data services            
            PaginationService.submit_edit_form_data(GLOBAL.API_URL+"domains/edit", edit_data,'','domain_page');
    ...
    $rootScope.data_active_inactive = function(item){
        $timeout(function() {
            PaginationService.change_active_inactive(GLOBAL.API_URL+"domains/change-status", item.id, item.e_status);
           ...
           else
           {
               ...
               bootbox.confirm("Are you sure you want to "+ massage+" selected record?", function(result) {
                if(result == true)
                {
                  ...
                  var responsePromise = $http.post(GLOBAL.API_URL+'domains/bulk-actions',{'id':$rootScope.ids,'action':$status});
                   responsePromise.success(function(data, status1, headers, config) {
    ...
    $rootScope.delete_btn = function(id)
    {
        bootbox.confirm("Are you sure you want to delete this record?", function(result) {
            if(result == true)
            {
                ...
                PaginationService.delete_data(GLOBAL.API_URL+"domains/domain-delete",id);
                setTimeout(function(){
                    $rootScope.$apply();
