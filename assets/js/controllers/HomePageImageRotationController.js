'use strict';
app.controller('HomePageImageRotationController', function($filter,$rootScope, $scope, $http, $timeout, PaginationService, GLOBAL, mySharedService, $state, $stateParams) {
    ...
    $scope.$on('$viewContentLoaded', function() {
        ...
        PaginationService.loading(GLOBAL.API_URL+"home-image-rotation","updated_at");
    });
    ...
    $rootScope.export_to_excel = function(fields){
        ...
        window.location.href = GLOBAL.API_URL+'home-image-rotation/export/'+$scope.someModelSerialized;
    }
    ...
    if($stateParams.id != "" && $stateParams.id != undefined) {
        if(mySharedService.edit_fields != undefined) {
            ...
            var responsePromise = $http.post(GLOBAL.API_URL+"home-image-rotation/data/"+$stateParams.id);
    ...
    $rootScope.frm_edit_open = function(item)
    {
        ...
        $timeout(function()
        {
            if( $scope.edit_fields.v_image != '')
            {
                ...
                $scope.$parent.$apply();
                $rootScope.imageDataURI = GLOBAL.SITE_URL+GLOBAL.HOME_PAGE_IMAGE_ROTATOR_PATH+ $scope.edit_fields.v_image;
        ...
        $timeout(function()
        {
            if( $scope.edit_fields.v_image != '') {
                ...
                $scope.$parent.$apply();
                $rootScope.imageDataURIGrid = GLOBAL.SITE_URL+GLOBAL.HOME_PAGE_IMAGE_ROTATOR_PATH+ $scope.edit_fields.v_image;
...
//                                }
    ...
    $rootScope.loadEditData = function(item){
        ...
        $state.go('/home-image-rotator-edit',{id: item.id});
    }
    ...
    $rootScope.submit_frm_edit = function(edit_fields){
        ...
        setTimeout(function(){
            if(form_valid('#frmEdit',"help-block")){
                ...
                PaginationService.submit_edit_form_data(GLOBAL.API_URL+"home-image-rotation/edit", edit_fields, '/home-image-rotator');
    ...
    $scope.delete_btn = function(id)
    {
        bootbox.confirm("Are you sure you want to delete this record?", function(result) {
            if(result == true)
            {
                ...
                PaginationService.delete_data(GLOBAL.API_URL+"home-image-rotation/delete",id);
                setTimeout(function(){
                    $rootScope.$apply();
                    setTimeout(function(){
                        angular.element('.alert-success').fadeOut(2000);
        ...
        {   
           ...
           else
           {
               ...
               bootbox.confirm("Are you sure you want to "+ massage+" selected record?", function(result) {
                if(result == true)
                {
                    var responsePromise = $http.post(GLOBAL.API_URL+'home-image-rotation/bulk-actions',{'id':$rootScope.ids,'action':$status});
                    responsePromise.success(function(data, status1, headers, config) {
                        if(data == "1"){
...
app.controller('HomePageImageRotationAddController', function($filter,$rootScope, $scope, $http, $timeout, PaginationService, GLOBAL, mySharedService, $state, $stateParams) {
        ...
        setTimeout(function(){
            if(form_valid('#frmAdd',"help-block")){
                ...
                if(button_pressed == 'New'){
                    PaginationService.submit_add_form_data(GLOBAL.API_URL+"home-image-rotation/add", add_fields);
                } else {
                    PaginationService.submit_add_form_data(GLOBAL.API_URL+"home-image-rotation/add", add_fields, '/home-image-rotator');
                }
